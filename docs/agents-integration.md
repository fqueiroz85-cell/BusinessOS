...
- **Modelo padrão Sonnet** (`AGENT_CLI_MODEL`), acima do Haiku do briefing e abaixo do Opus. O briefing é síntese curta de respostas que o founder acabou de dar, acionada por um botão — Haiku dá conta. Um agente manda o negócio inteiro no prompt, então rodar vários numa sessão consome cota de forma perceptível; Sonnet corta bem esse custo sem perder muito neste tipo de tarefa. Suba para `opus` quando quiser o máximo de qualidade em um item específico.
